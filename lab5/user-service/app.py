...
async def find_user_by_login(
    ...
    cached_user = await redis.get(f"user:{login}")
    if cached_user:
        logger.info(f"User found in cache: {login}")
        return {"username": login, "source": "cache"}
    ...
    await redis.setex(f"user:{login}", 300, f"{user['id']},{user['created_at']}")
    logger.info(f"User found and cached: {login}")
    return {"id": user['id'], "username": user['username'], "created_at": user['created_at']}


@app.get("/users/search", summary="Find users by name mask")
async def find_user_by_name_mask(
    nameMask: str,
    username: str = Depends(verify_token),
    db=Depends(get_db),
    redis: aioredis.Redis = Depends(get_redis)
    ...
    key = f"search:{nameMask}"
    cached_result = await redis.get(key)
    if cached_result:
        logger.info(f"Users found in cache for mask: {nameMask}")
    ...
    await redis.setex(key, 300, str(result))
    return result
...
async def update_user(
    ...
    logger.info(f"User updated successfully: {username}, cache cleared.")
...
async def delete_user(
    ...
    await redis.delete(f"user:{username}")
    logger.info(f"User deleted successfully: {username}, cache cleared.")
